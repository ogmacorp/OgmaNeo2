...
void SparseCoder::forward(
    ...
    for (int hc = 0; hc < _hiddenSize.z; hc++) {
        ...
        for (int vli = 0; vli < _visibleLayers.size(); vli++) {
            ...
            sum += vl._weights.multiplyThreshOHVs(*inputCs[vli], hiddenIndex, vld._size.z, _thresh);
        }
...
void SparseCoder::learn(
    ...
    for (int vc = 0; vc < vld._size.z; vc++) {
        ...
        float sum = vl._weights.multiplyThreshOHVsT(_hiddenCs, visibleIndex, _hiddenSize.z, _thresh) / std::max(1, vl._weights.countT(visibleIndex) / _hiddenSize.z);
